...
if ($result = pg_query($link, $req)) {
  while ($ligne = pg_fetch_assoc($result)) {
    ...
    $properties = ["id" => $id, "amenageur"=> $amenageur, "operateur" => $operateur, "enseigne" => $enseigne, "id_station" => $id_station, "nom" => $nom_station, "adresse" => $adresse, "code_insee" => $code_insee, "nb_prise" => $nb_prise, "id_prise" => $id_prise, "puissance" => $puissance, "type_prise" => $type_prise, "cout" => $cout, "access" => $access, 'geometry'=> array( 'type' => 'Point', 'coordinates' => array( $lat, $lng))];
