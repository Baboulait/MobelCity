...
$req = "SELECT * from centroidesetcotes_final where unx >= $left and troisx <= $right and uny <= $top and troisy >= $bottom ORDER BY nbborne DESC ";
...
if ($result = pg_query($link, $req)) {
  while ($ligne = pg_fetch_assoc($result)) {
    ...
    $properties = ["id" => $id,"pop"=> $pop, "libgeo" => $libgeo,"borneNormale" => $nbnormale, "borneSemiRapide" => $nbsemirapi, "borneRapide" => $rapide, "centrex" => $centrex, "centrey" => $centrey, "nbborne" => $nbborne, 'geometry'=>array( 'type' => 'Polygon', 'coordinates' => array([$uny, $unx],[$deuxy,$deuxx],[$troisy,$troisx],[$quatrey,$quatrex]))];
